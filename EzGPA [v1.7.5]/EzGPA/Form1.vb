Public Class Form1
    ...
    '--------------------------Subject Credits
    Const ChineseCredits As Double = 4.5
    Const EnglishCredits As Double = 7.5
    Const MathCredits As Double = 7
    Const PhysicsCredits As Double = 4
    Const HistoryCredits As Double = 3.5
    Const ChemistryCredits As Double = 3
    Const ITEnglishWritingCredits As Double = 1.5
    Const TotalCredits As Double = 31
    '--------------------------Application Info
    Const VersionNumber As String = "1.7.5"
    Const MajorEdit As String = "a"
    Const ChangeLogEN As String = "Rewrote score calculation to make the code easier to read/modify. " & _
                                  "It should have no impact on performance, and it will allow for faster updates in the future."
    Const ChangeLogCN As String = "???"
    ...
    Function FinalGPA()
        ...
        Return Val(String.Format("{0:0.000000000000}", _
                                  ((ChineseGPA * ChineseCredits) _
                                 + (EnglishGPA * EnglishCredits) _
                                 + (MathGPA * MathCredits) _
                                 + (PhysicsGPA * PhysicsCredits) _
                                 + (HistoryGPA * HistoryCredits) _
                                 + (ChemistryGPA * ChemistryCredits) _
                                 + (ITEnglishWritingGPA * ITEnglishWritingCredits)) _
                                 / TotalCredits)) 'ROUND
    End Function
    ...
    Private Sub About_Click(sender As System.Object, e As System.EventArgs) Handles About.Click
        If ApplicationLanguage = "English" Then
            MsgBox("EzGPA v" & VersionNumber & Environment.NewLine & _
                   "Copyright © 2011-2012 Andrew Sun <crossbowffs@live.com>" & Environment.NewLine & _
                   "Built on: " & BuildTime() & Environment.NewLine & Environment.NewLine & _
                   "v" & VersionNumber & " changelog: " & ChangeLogEN, MsgBoxStyle.Information)
        ElseIf ApplicationLanguage = "Chinese" Then
            MsgBox("EzGPA v" & VersionNumber & Environment.NewLine & _
                   "版权所有 © 2011-2012 Andrew Sun <crossbowffs@live.com>" & Environment.NewLine & _
                   "编辑日期: " & BuildTime() & Environment.NewLine & Environment.NewLine & _
                   "v" & VersionNumber & " 更改日志: " & ChangeLogCN, MsgBoxStyle.Information)
        End If
    End Sub
