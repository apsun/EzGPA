...
namespace EzGPA
{
    public partial class Form1 : Form
    {
        ...
        //Registry constants (Why did I even make these? I could have just used strings...)
        const int REG_READ = 1;
        const int REG_WRITE = 2;
        //Variables used for internal checks
        bool AppCrashed = false; //True means the app crashed and we should not save registry settings
        bool AppClosed = false; //True means the app was closed normally and we should not clear scores
        //Version-specific strings
        const string Version = "2.0_Alpha 7";
        const string Changelog = "Form1.cs - Now with 789 lines of code!";

        public Form1()
        {
            //Can't touch this! (No, seriously, don't touch this.)
            InitializeComponent();
        }
        ...
        private void ReadWriteRegistryKeys(int action)
        {
            try
            {
                ...
                if (action == REG_READ)
                {
                    ...
                    //General settings
                    LoadRegistrySettings(RegKeyMain, "Grade", 0, ref tabControl1);
                ...
                if (action == REG_WRITE)
                {
                    ...
                    //General settings
                    SaveRegistrySettings(RegKeyMain, "Grade", tabControl1.SelectedIndex + 9); //+ 9 because we start at 9th grade
        ...
        private void LoadRegistrySettings(RegistryKey regKey, string value, int defaultValue, ref TabControl cont)
        {
            //TabControls
            try
            {
                cont.SelectedIndex = Convert.ToInt16(regKey.GetValue(value, defaultValue)) - 9; //-9 because we start at 9th grade
            }
            catch //(Exception ex)
            {
                //Don't need to crash the app for this one either
                //CrashMe(ex);
            }
        }
        ...
        {
            //Returns the base GPA of a subject
            if (score >= 93)
            {
                return 4;
            }
        ...
        private double CalcFinalGPA()
        {
            ...
            if (tabControl1.SelectedIndex == 0)
            {
                ...
                //This is because we are updating the text every tick, and not after the user confirms their selection.
                //This will prevent incorrect GPAs when hoving over a native/non-native option.
                ...
                finalGPA = (ChineseGPA   * Convert.ToInt32(G9_ChineseEnabledCheckBox.Checked) 
        ...
        private bool AllScoresEntered()
        {
            ...
            {
                if ((G10_ChineseScoreTextBox.Text   == "" && G10_ChineseEnabledCheckBox.Checked)   ||
                    (G10_EnglishScoreTextBox.Text   == "" && G10_EnglishEnabledCheckBox.Checked)   ||
                    (G10_MathScoreTextBox.Text      == "" && G10_MathEnabledCheckBox.Checked)      ||
                    (G10_PhysicsScoreTextBox.Text   == "" && G10_PhysicsEnabledCheckBox.Checked)   ||
                    (G10_ChemistryScoreTextBox.Text == "" && G10_ChemistryEnabledCheckBox.Checked) ||
                    (G10_EconomicsScoreTextBox.Text == "" && G10_EconomicsEnabledCheckBox.Checked) ||
                    (G10_Optional1ScoreTextBox.Text == "" && G10_Optional1EnabledCheckBox.Checked) ||
                    (G10_Optional2ScoreTextBox.Text == "" && G10_Optional2EnabledCheckBox.Checked))
                {
                    return false;
                }
            }
        ...
        private bool AtLeastOneSubjectEnabled()
        {
            //Grade 9
            if (tabControl1.SelectedIndex == 0)
            {
                if (G9_ChineseEnabledCheckBox.Checked   ||
                    G9_EnglishEnabledCheckBox.Checked   ||
                    G9_MathEnabledCheckBox.Checked      ||
                    G9_PhysicsEnabledCheckBox.Checked   ||
                    G9_HistoryEnabledCheckBox.Checked   ||
                    G9_ChemistryEnabledCheckBox.Checked ||
                    G9_OptionalEnabledCheckBox.Checked)
                {
                    return true;
                }
            }
            ...
            {
                if (G10_ChineseEnabledCheckBox.Checked   ||
                    G10_EnglishEnabledCheckBox.Checked   ||
                    G10_MathEnabledCheckBox.Checked      ||
                    G10_PhysicsEnabledCheckBox.Checked   ||
                    G10_ChemistryEnabledCheckBox.Checked ||
                    G10_EconomicsEnabledCheckBox.Checked ||
                    G10_Optional1EnabledCheckBox.Checked ||
                    G10_Optional2EnabledCheckBox.Checked)
                {
                    return true;
                }
            }
        ...
        private void TextBox_GotFocus(object sender, EventArgs e)
        {
            ...
            //The following check is to prevent debug messageboxes from accidentally triggering this event
            if (!AppClosed && !AppCrashed)
            {
        ...
        private void TextBox_LostFocus(object sender, EventArgs e)
        {
            ...
            if (s.Text != "")
            {
                //VB.NET equivalent: s.Text = Val(s.Text)
        ...
        {
            //No subjects enabled
            if (!AtLeastOneSubjectEnabled())
            {
                GPALabel.Text = "You must select at least one subject!";
            }

            //Some enabled subjects are blank
        ...
        {
            if (AllScoresEntered() && AtLeastOneSubjectEnabled())
            {
                try
                {
